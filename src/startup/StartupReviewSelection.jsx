...
import { 
  Check, X, Loader, Eye, Info, Globe, Map, Building, Phone, 
  Mail, Calendar, Briefcase, Users, DollarSign, Award, 
  Hash, Clock, Filter, Download, ArrowUpDown, AlertTriangle, 
...
export default function EnhancedStartupReviewSection() {
  ...
  useEffect(() => {
    if (startups.length > 0) {
      setUniqueIndustries([...new Set(startups.map(startup => startup.industry).filter(Boolean))]);
      setUniqueCountries([...new Set(startups.map(startup => startup.country).filter(Boolean))]);
      setUniqueStatuses([...new Set(startups.map(startup => startup.status).filter(Boolean))]);
    }
  }, [startups]);
  ...
  const exportToCSV = () => {
    const headers = [
      "Company Name", "Industry", "Founded Date", "Status", 
      "City", "Country", "Contact Email", "Phone Number"
    ];
  ...
  const filteredStartups = useMemo(() => {
    ...
    if (filters.industry) {
      result = result.filter(startup => startup.industry === filters.industry);
    }
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200">
      ...
      {filtersVisible && (
        <div className="p-4 border-b bg-blue-50">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              <label htmlFor="date-range-end-filter" className="block text-sm font-medium text-gray-700 mb-1">
                Founded Before
              </label>
              <input
      ...
      <div className="p-4 flex flex-col sm:flex-row justify-between items-center border-b">
        ...
        <div className="flex items-center text-sm text-gray-600">
          <span>
            Showing {filteredStartups.length ? (currentPage - 1) * itemsPerPage + 1 : 0}-
      ...
      {actionVisible && (
        <div 
          className={`transition-all duration-300 fixed bottom-4 right-4 p-4 rounded-lg shadow-lg ${
            actionResult.success ? 'bg-green-100 border-l-4 border-green-500' : 'bg-red-100 border-l-4 border-red-500'
          }`}
        >
          <div className="flex">
            ...
            <div className="ml-3">
              <p className={`text-sm font-medium ${actionResult.success ? 'text-green-800' : 'text-red-800'}`}>
      ...
      {filteredStartups.length === 0 ? (
        <div className="p-10 text-center text-gray-500">
          ...
          <p className="text-lg mb-2">No startups found matching your criteria.</p>
          <p className="text-sm text-gray-400">Try adjusting your filters or search terms.</p>
          {filters.search || filters.industry || filters.status || filters.country || filters.dateRange.start || filters.dateRange.end ? (
            <button
              onClick={resetFilters}
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-gray-100 text-gray-700">
                <th className="p-3 text-left text-sm font-medium border-b" onClick={() => handleSort("companyName")}>
                  <div className="flex items-center cursor-pointer group">
                    Company
                    <ArrowUpDown size={14} className={`ml-1 ${sortConfig.key === "companyName" ? 'text-blue-600' : 'text-gray-400 group-hover:text-gray-600'}`} />
                  </div>
                </th>
                <th className="p-3 text-left text-sm font-medium border-b" onClick={() => handleSort("industry")}>
                  <div className="flex items-center cursor-pointer group">
                    Industry
                    <ArrowUpDown size={14} className={`ml-1 ${sortConfig.key === "industry" ? 'text-blue-600' : 'text-gray-400 group-hover:text-gray-600'}`} />
                  </div>
                </th>
                <th className="p-3 text-left text-sm font-medium border-b" onClick={() => handleSort("foundedDate")}>
                  <div className="flex items-center cursor-pointer group">
                    Founded
                    <ArrowUpDown size={14} className={`ml-1 ${sortConfig.key === "foundedDate" ? 'text-blue-600' : 'text-gray-400 group-hover:text-gray-600'}`} />
                  </div>
                </th>
                <th className="p-3 text-left text-sm font-medium border-b" onClick={() => handleSort("status")}>
                  <div className="flex items-center cursor-pointer group">
                    Status
                    <ArrowUpDown size={14} className={`ml-1 ${sortConfig.key === "status" ? 'text-blue-600' : 'text-gray-400 group-hover:text-gray-600'}`} />
                  </div>
                </th>
                <th className="p-3 text-left text-sm font-medium border-b" onClick={() => handleSort("country")}>
                  <div className="flex items-center cursor-pointer group">
                    Location
                    <ArrowUpDown size={14} className={`ml-1 ${sortConfig.key === "country" ? 'text-blue-600' : 'text-gray-400 group-hover:text-gray-600'}`} />
                  </div>
                </th>
            ...
            <tbody>
              {paginatedStartups.map((startup) => (
                <tr key={startup.id} className="hover:bg-blue-50 border-b text-gray-500 transition-colors">
                  <td className="p-3">
                    <div className="font-medium text-gray-900">{startup.companyName}</div>
                  ...
                  <td className="p-3">
                    ...
                    {startup.businessActivity && (
                      <div className="text-xs text-gray-500 mt-1">{startup.businessActivity}</div>
                    )}
                  </td>
                  <td className="p-3">
                    ...
                    <div className="text-xs text-gray-500 mt-1">{startup.typeOfCompany || "N/A"}</div>
                  </td>
                  <td className="p-3">
                  ...
                  <td className="p-3">
                    ...
                    <div className="text-xs text-gray-500">{startup.country || "N/A"}</div>
                  </td>
                  <td className="p-3">
                    <div className="flex items-center text-sm">
                  ...
                  <td className="p-3">
                    <div className="flex justify-center space-x-2">
                      ...
                      <button
                        ...
                        disabled={isActionLoading}
                      >
                        {isActionLoading && selectedStartup?.id === startup.id ? (
            ...
            <div className="overflow-y-auto p-6 flex-grow">
              ...
              <div className="flex items-center justify-between mb-6 bg-blue-50 p-3 rounded-lg border border-blue-100">
                <div className="flex items-center">
                  ...
                  <span className="text-sm text-blue-800">Status: <strong>{selectedStartup.status}</strong></span>
                </div>
                <div className="flex items-center space-x-4">
                  <div className="flex items-center">
                    <Eye size={16} className="text-blue-700 mr-1" />
                    <span className="text-sm text-blue-800">Views: {selectedStartup.viewsCount || 0}</span>
                  </div>
                  <div className="flex items-center">
                    <Calendar size={16} className="text-blue-700 mr-1" />
                    <span className="text-sm text-blue-800">Submitted: {formatDate(selectedStartup.createdAt || selectedStartup.foundedDate)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                  <div className="bg-white p-5 rounded-lg border shadow-sm">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="text-gray-800 font-medium">{selectedStartup.companyName}</p>
                      </div>
                      
                      <div>
                        <p className="text-sm font-medium text-gray-500">Type of Company</p>
                        <p className="text-gray-800">{selectedStartup.typeOfCompany || "N/A"}</p>
                      </div>
                      
                      <div>
                        <p className="text-sm font-medium text-gray-500">Industry</p>
                        <p className="text-gray-800">{selectedStartup.industry}</p>
                      </div>
                      
                      <div>
                        <p className="text-sm font-medium text-gray-500">Business Activity</p>
                        <p className="text-gray-800">{selectedStartup.businessActivity || "N/A"}</p>
                      </div>
                      
                      <div>
                        <p className="text-sm font-medium text-gray-500">Founded Date</p>
                        <p className="text-gray-800">{formatDate(selectedStartup.foundedDate)}</p>
                      </div>
                      
                      <div>
                        <p className="text-sm font-medium text-gray-500">Company Size</p>
                        <p className="text-gray-800">{selectedStartup.numberOfEmployees || "N/A"} employees</p>
                      </div>
                      
                      <div>
                        <p className="text-sm font-medium text-gray-500">Operating Hours</p>
                        <p className="text-gray-800">{selectedStartup.operatingHours || "N/A"}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white p-5 rounded-lg border shadow-sm">
                    ...
                    <p className="text-gray-700 whitespace-pre-line">
                      {selectedStartup.companyDescription || "No description provided."}
                    </p>
                  </div>
                  ...
                  <div className="bg-white p-5 rounded-lg border shadow-sm">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="text-gray-800">{selectedStartup.fundingStage || "N/A"}</p>
                      </div>
                      
                      <div>
                        <p className="text-sm font-medium text-gray-500">Number of Funding Rounds</p>
                        <p className="text-gray-800">{selectedStartup.numberOfFundingRounds || "N/A"}</p>
                      </div>
                      
                      <div>
                        <p className="text-sm font-medium text-gray-500">Active Startups</p>
                        <p className="text-gray-800">{selectedStartup.numberOfActiveStartups || "N/A"}</p>
                      </div>
                      
                      <div>
                        <p className="text-sm font-medium text-gray-500">New Startups This Year</p>
                        <p className="text-gray-800">{selectedStartup.numberOfNewStartupsThisYear || "N/A"}</p>
                      </div>
                      
                      <div>
                        <p className="text-sm font-medium text-gray-500">Startup Survival Rate</p>
                        <p className="text-gray-800">
                          {selectedStartup.startupSurvivalRate ? `${selectedStartup.startupSurvivalRate}%` : "N/A"}
                        </p>
                      </div>
                      ...
                      <div>
                        ...
                        <p className="text-gray-800">
                          {selectedStartup.averageStartupGrowthRate ? `${selectedStartup.averageStartupGrowthRate}%` : "N/A"}
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="space-y-6">
                  <div className="bg-white p-5 rounded-lg border shadow-sm">
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <p className="text-gray-800">{selectedStartup.contactEmail}</p>
                      </div>
                      ...
                      <div>
                        ...
                        <p className="text-gray-800">{selectedStartup.phoneNumber || "N/A"}</p>
                      </div>
                      ...
                      <div>
                        ...
                        <p className="text-gray-800">
                          {selectedStartup.city || "N/A"}, {selectedStartup.province || ""}, {selectedStartup.country || ""}
                        </p>
                        <p className="text-gray-500 text-sm">
                          {selectedStartup.streetAddress || ""} {selectedStartup.postalCode || ""}
                        </p>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="space-y-3">
                      {selectedStartup.website && (
                        <div>
                          ...
                          <a href={selectedStartup.website} target="_blank" rel="noopener noreferrer" 
                             className="text-blue-600 hover:underline break-words">{selectedStartup.website}</a>
                        </div>
                      )}
                      ...
                      {selectedStartup.linkedIn && (
                        <div>
                          ...
                          <a href={selectedStartup.linkedIn} target="_blank" rel="noopener noreferrer"
                             className="text-blue-600 hover:underline break-words">
                            {selectedStartup.linkedIn.replace("https://www.linkedin.com/company/", "")}
                          </a>
                        </div>
                      )}
                      ...
                      {selectedStartup.facebook && (
                        <div>
                          ...
                          <a href={selectedStartup.facebook} target="_blank" rel="noopener noreferrer"
                             className="text-blue-600 hover:underline break-words">
                            {selectedStartup.facebook.replace("https://www.facebook.com/", "")}
                          </a>
                        </div>
                      )}
                      ...
                      {selectedStartup.twitter && (
                        <div>
                          ...
                          <a href={selectedStartup.twitter} target="_blank" rel="noopener noreferrer"
                             className="text-blue-600 hover:underline break-words">
                            {selectedStartup.twitter.replace("https://twitter.com/", "@")}
                          </a>
                        </div>
                      )}
                      ...
                      {selectedStartup.instagram && (
                        <div>
                          ...
                          <a href={selectedStartup.instagram} target="_blank" rel="noopener noreferrer"
                             className="text-blue-600 hover:underline break-words">
                            {selectedStartup.instagram.replace("https://www.instagram.com/", "@")}
                          </a>
                        </div>
                      )}
                      
                      {!selectedStartup.website && !selectedStartup.linkedIn && !selectedStartup.facebook && 
                       !selectedStartup.twitter && !selectedStartup.instagram && (
                        <p className="text-gray-500 italic">No web presence information provided.</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="bg-white p-5 rounded-lg border shadow-sm">
                    ...
                    <div className="space-y-3">
                      <div>
                        <p className="text-sm font-medium text-gray-500">Number of Mentors</p>
                        <p className="text-gray-800">{selectedStartup.numberOfMentorsOrAdvisorsInvolved || "N/A"}</p>
                      </div>
                      
                      <div>
                        <p className="text-sm font-medium text-gray-500">Incubation Programs</p>
                        <p className="text-gray-800">{selectedStartup.numberOfStartupsInIncubationPrograms || "N/A"}</p>
                      </div>
                      
                      <div>
                        <p className="text-sm font-medium text-gray-500">Public-Private Partnerships</p>
                        <p className="text-gray-800">{selectedStartup.publicPrivatePartnershipsInvolvingStartups || "N/A"}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white p-5 rounded-lg border shadow-sm">
                    ...
                    <div className="space-y-4">
                      <div className="flex items-start">
                        ...
                        <div>
                          <p className="text-sm font-medium text-gray-700">Startup Submitted</p>
                          <p className="text-xs text-gray-500">{formatDate(selectedStartup.createdAt || selectedStartup.foundedDate)}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-start">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500">{formatDate(selectedStartup.emailVerifiedAt || selectedStartup.foundedDate)}</p>
                        </div>
                      </div>
                      
                      <div className="flex items-start">
                        <div className={`${
                          selectedStartup.status === "Approved" ? "bg-green-100" : 
                          selectedStartup.status === "Rejected" ? "bg-red-100" : 
                          "bg-gray-100"
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500">
                            {selectedStartup.status === "Approved" || selectedStartup.status === "Rejected" ? 
                              formatDate(selectedStartup.updatedAt || selectedStartup.foundedDate) : 
                              "Pending"}
                          </p>
                        </div>
                      </div>
