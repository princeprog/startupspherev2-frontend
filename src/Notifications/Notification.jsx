...
import { FaBell, FaCheckCircle, FaRegCircle, FaRegTrashAlt, FaSearch, FaSortAmountDown, FaSortAmountUp } from 'react-icons/fa';
    ...
    if (this.state.hasError) {
      return (
        <div className="p-8 text-center">
          ...
          <p className="text-gray-600 mb-4">We had trouble loading your notifications</p>
          <button 
...
function NotificationComponent() {
    ...
    const transformNotificationData = (item) => {
        return {
            ...
            sender: item.startup ? {
                ...
                avatar: item.startup.photo ? `${import.meta.env.VITE_BACKEND_URL}/startups/${item.startup.id}/photo` : null
            } : null,
        ...
        try {
            ...
            if (result.success) {
                ...
                if (Array.isArray(result.data)) {
                    notificationData = result.data.map(item => transformNotificationData(item));
                } else if (result.data) {
    ...
    const getRandomName = () => {
        const firstNames = ['Alex', 'Jordan', 'Taylor', 'Morgan', 'Casey', 'Riley', 'Jamie', 'Avery', 'Cameron', 'Skyler'];
        const lastNames = ['Smith', 'Johnson', 'Williams', 'Jones', 'Brown', 'Davis', 'Miller', 'Wilson', 'Moore', 'Taylor'];
        return `${firstNames[Math.floor(Math.random() * firstNames.length)]} ${lastNames[Math.floor(Math.random() * lastNames.length)]}`;
    };
    
    const getRandomStartupName = () => {
        const prefixes = ['Tech', 'Inno', 'Future', 'Smart', 'Eco', 'Next', 'Quantum', 'Cyber', 'Digital', 'Meta'];
        const suffixes = ['Corp', 'Labs', 'Tech', 'AI', 'Solutions', 'Systems', 'Hub', 'Works', 'Wave', 'Sphere'];
    ...
    useEffect(() => {
        try {
            ...
            if (activeTypeFilter !== 'all') {
                result = result.filter(notif => notif && notif.type === activeTypeFilter);
            }
            ...
            if (searchQuery) {
                ...
                result = result.filter(notif => 
                    ...
                    ((notif.content && notif.content.toLowerCase().includes(query)) || 
                     (notif.sender && notif.sender.name && notif.sender.name.toLowerCase().includes(query)))
                );
            }
    ...
    const handleToggleRead = async (id, currentReadStatus) => {
        try {
            ...
            toast.success(`Notification marked as ${currentReadStatus ? 'unread' : 'read'}`);
        } catch (error) {
    ...
    const markAllAsRead = async () => {
        try {
            const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/notifications/my/markAllRead`, {
                method: 'PUT',
                credentials: 'include',
                headers: {
                    'Content-Type': 'application/json'
    ...
    const getNotificationIcon = (type) => {
        try {
            switch(type) {
                case 'connection':
                    return <div className="bg-blue-100 p-2 rounded-full"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-blue-600" viewBox="0 0 20 20" fill="currentColor"><path d="M9 6a3 3 0 11-6 0 3 3 0 016 0zM17 6a3 3 0 11-6 0 3 3 0 016 0zM12.93 17c.046-.327.07-.66.07-1a6.97 6.97 0 00-1.5-4.33A5 5 0 0119 16v1h-6.07zM6 11a5 5 0 015 5v1H1v-1a5 5 0 015-5z" /></svg></div>;
                case 'message':
                    return <div className="bg-green-100 p-2 rounded-full"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-green-600" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M18 10c0 3.866-3.582 7-8 7a8.841 8.841 0 01-4.083-.98L2 17l1.338-3.123C2.493 12.767 2 11.434 2 10c0-3.866 3.582-7 8-7s8 3.134 8 7zM7 9H5v2h2V9zm8 0h-2v2h2V9zM9 9h2v2H9V9z" clipRule="evenodd" /></svg></div>;
                case 'update':
                    return <div className="bg-amber-100 p-2 rounded-full"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-amber-600" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M4 2a1 1 0 011 1v2.101a7.002 7.002 0 0111.601 2.566 1 1 0 11-1.885.666A5.002 5.002 0 005.999 7H9a1 1 0 010 2H4a1 1 0 01-1-1V3a1 1 0 011-1zm.008 9.057a1 1 0 011.276.61A5.002 5.002 0 0014.001 13H11a1 1 0 110-2h5a1 1 0 011 1v5a1 1 0 11-2 0v-2.101a7.002 7.002 0 01-11.601-2.566 1 1 0 01.61-1.276z" clipRule="evenodd" /></svg></div>;
                case 'system':
                    return <div className="bg-purple-100 p-2 rounded-full"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-purple-600" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" /></svg></div>;
                default:
                    return <div className="bg-gray-100 p-2 rounded-full"><FaBell className="h-5 w-5 text-gray-600" /></div>;
            }
        } catch (error) {
    ...
    const countByType = useMemo(() => {
        ...
        typeFilters.forEach(filter => {
            if (filter.id !== 'all') {
                counts[filter.id] = notifications.filter(n => n && n.type === filter.id).length;
            }
        });
        ...
        return (
            <div className="min-h-screen bg-gray-50 flex items-center justify-center">
                <div className="text-center p-8 max-w-md bg-white rounded-lg shadow-lg">
                    <FaBell className="h-12 w-12 text-gray-400 mx-auto mb-4" />
                    <h2 className="text-xl font-semibold text-gray-800 mb-2">Unable to load notifications</h2>
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
                        ...
                        <div className="flex flex-col sm:flex-row w-full md:w-auto space-y-3 sm:space-y-0 sm:space-x-3">
                                ...
                                {showFilterMenu && (
                                    <div className="absolute right-0 mt-2 w-64 bg-white rounded-lg shadow-lg z-10 overflow-hidden border border-gray-100">
                                        <div className="p-3 bg-gray-50 border-b border-gray-100">
                                            <h3 className="font-medium text-gray-800">Filter Notifications</h3>
                                        </div>
                                            ...
                                            <div className="space-y-2">
                                                {statusFilters.map(filter => (
                                                    <button
                                                        ...
                                                        className={`block text-black w-full text-left px-2 py-1 rounded flex justify-between items-center ${activeFilter === filter.id ? 'bg-blue-50 text-blue-700' : 'hover:bg-gray-50'}`}
                                                    >
                                            ...
                                            <div className="space-y-2">
                                                {typeFilters.map(filter => (
                                                    <button
                                                        ...
                                                        className={`text-black block w-full text-left px-2 py-1 rounded flex justify-between items-center ${activeTypeFilter === filter.id ? 'bg-blue-50 text-blue-700' : 'hover:bg-gray-50'}`}
                                                    >
                                                        <span>{filter.name}</span>
                                                        <span className="bg-gray-100 text-xs px-1.5 py-0.5 rounded-full">
                                                            {countByType[filter.id] || 0}
                                        ...
                                        <div className="p-3 border-t border-gray-100">
                                            <p className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-2">Sort</p>
                                            <button
                                                onClick={() => {
                                                    toggleSortOrder();
                                                    setShowFilterMenu(false);
                            ...
                            <button 
                                ...
                                disabled={!filteredNotifications || !filteredNotifications.some(n => n && !n.isRead)}
                            >
                    ...
                    <div className="flex flex-wrap gap-2 mt-2 sm:mt-0">
                        {typeFilters.map(filter => (
                            ...
                            >
                                <span>{filter.name}</span>
                                <span className={`ml-2 px-1.5 py-0.5 text-xs rounded-full ${
                                    activeTypeFilter === filter.id ? 'bg-blue-200 text-blue-800' : 'bg-gray-100 text-gray-600'
                                }`}>
                                    {countByType[filter.id] || 0}
                                </span>
                            </button>
                    ...
                    {loading ? (
                        ...
                        <ul className="divide-y divide-gray-200">
                            {filteredNotifications.map((notification) => (
                                notification && <li 
                                    key={notification.id} 
                                    className={`relative transition-all ${notification.isRead ? 'bg-white' : 'bg-blue-50'} hover:bg-gray-50`}
                                >
                                    <div className="px-4 py-5 sm:px-6 flex items-start">
                                        ...
                                        <div className="flex-shrink-0 self-center flex">
                                            ...
                                            >
                                                {notification.isRead ? <FaRegCircle className="h-5 w-5" /> : <FaCheckCircle className="h-5 w-5" />}
                                            </button>
                        ...
                        <div className="p-12 flex flex-col items-center justify-center">
                            ...
                            {(activeFilter !== 'all' || activeTypeFilter !== 'all' || searchQuery) && (
                                <button
                                    onClick={() => {
                                        setActiveFilter('all');
